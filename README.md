...
- **Real-time 3D preview** — orbit camera, adjustable height scale, adjustable mesh density (256² – 2048², independent of bake resolution), 6 shading modes (Texture, Flat, Clay, Normals, AO, Wire)
...
- **Autosave** to local storage + manual save/load as `.json` graph files
- **Zero backend** — a single HTML file; open it or serve it statically

## Quick start

STRATA has no build step. Because it loads Three.js as an ES module, browsers won't run it from a bare `file://` URL — serve the folder locally instead:
...
Then open `http://localhost:8080/Strata_10.0.html` in a recent Chromium/Firefox/Safari (WebGL2 + ES modules required). An internet connection is needed on first load — Three.js, the UI font, and icons are pulled from CDNs (unpkg, Google Fonts).
...
Generates a 0–1 erosion-resistance mask (random patches, terrain-following rock layers, or both) and feeds an optional **Hardness** input on Hydraulic, Thermal, Aeolian, and Stream Power erosion. `0` = erodes normally, `1` = fully resistant. Used to carve mesas, cliff bands, and waterfalls/knickpoints where a hard layer meets softer rock, instead of uniform erosion everywhere.
...
- Tab must stay focused/visible during a heavy rebuild — background-tab timer throttling can slow down (not break) in-progress builds on very large graphs or high resolutions
- No undo/redo yet
